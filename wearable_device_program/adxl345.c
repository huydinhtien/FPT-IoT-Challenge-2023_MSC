...
void init_adxl(){
  ...
  uint16_t value = i2c_Read(ADXL345_REG_DEVID);
            ...
            // Set an LED OFF if Master can read the fix address 's adxl345 successfully
            if (value == DEVICE_ID)
            {
...
// assign x, y, z to array xyz[0], xyz[1], xyz[3] respectively
void read_acc(int *xyz){
  read_accel_data(xyz, xyz + 1, xyz + 2);
}
// transfer to physical value (m/s^2)
void get_acc(double *xyz){
  int xyz_int[3];
  int i;
  double gains[3];
  //measure three physical values
  gains[0] = 0.00376390;
  gains[1] = 0.00376009;
  gains[2] = 0.00349265;
  read_acc(xyz_int);
